...
std::size_t skip_ws(const std::string_view & line, std::size_t i)
{
...
double parse_arg(const std::string_view & line, std::size_t & i)
{
    double res = 0;
    std::size_t count = 0;
    bool good = true;
...
double unary(const double current, const Op op)
{
    ...
    case Op::FAC:
        double integral;
        if (current >= 0 && std::modf(current, &integral) <= eps) {
            return std::floor(std::tgamma(current + 1));
        }
        else {
