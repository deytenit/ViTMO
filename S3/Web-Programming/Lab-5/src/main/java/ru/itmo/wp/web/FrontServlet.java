...
public class FrontServlet extends HttpServlet {
    ...
    private static final Pattern LANG_REGEX = Pattern.compile("^[a-z]{2}$");
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        final Map<Class<?>, Object> methodParamReflection = Map.of(
        ...
        Method method = null;
        for (Class<?> prototype = pageClass; method == null && prototype != null; prototype = prototype.getSuperclass()) {
            method = Arrays.stream(prototype.getDeclaredMethods())
                    .filter(x ->
                            x.getName().equals(route.getAction())
                            && Arrays.stream(x.getParameterTypes()).allMatch(methodParamReflection::containsKey)
                    )
        ...
        Template template;
        try {
            String langPrefix = "_" + request.getSession().getAttribute("lang");
    ...
    private void processLanguage(HttpServletRequest request) {
        final String param = request.getParameter("lang");

        if (param != null && LANG_REGEX.matcher(param).matches()) {
