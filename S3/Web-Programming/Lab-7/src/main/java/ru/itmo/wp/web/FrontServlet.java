...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method before = findPageMethod(pageClass, "before");
        Method method = findPageMethod(pageClass, route.getAction());
        Method after = findPageMethod(pageClass, "after");

        if (before == null || after == null) {
            throw new ServletException("Page class must have declared method: 'before', 'after'.");
        }
        if (method == null) {
            throw new NotFoundException();
        }
        ...
        try {
            ...
            before.invoke(page, request, !json ? view : new HashMap<>());
            method.invoke(page, request, view);
            after.invoke(page, request, !json ? view : new HashMap<>());
    ...
    private Method findPageMethod(Class<?> pageClass, String name) {
        Method method = null;
        for (Class<?> prototype = pageClass; method == null && prototype != null; prototype = prototype.getSuperclass()) {
            try {
                method = prototype.getDeclaredMethod(name, HttpServletRequest.class, Map.class);
